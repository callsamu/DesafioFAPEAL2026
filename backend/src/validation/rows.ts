...
export const VARIABLES_BY_SOURCE: Record<z.infer<typeof SourceEnum>, string[]> = {
...
export const LEVELS_WITH_PERFORMANCE_RATE = new Set(['Ensino Fundamental', 'Ensino Médio']);

export const PERFORMANCE_RATES = ['Taxa de Aprovação', 'Taxa de Reprovação', 'Taxa de Abandono'];
...
type RowShape = z.infer<typeof rowSchema>;

function checkVariableForSource(row: RowShape, ctx: z.RefinementCtx) {
...
function checkPerformanceRateLevel(row: RowShape, ctx: z.RefinementCtx) {
  if (
    PERFORMANCE_RATES.includes(row.variavel) &&
...
function checkDemographicRules(row: RowShape, ctx: z.RefinementCtx) {
...
function checkRateValueRange(row: RowShape, ctx: z.RefinementCtx) {
...
export const RowSchema = rowSchema.superRefine((row, ctx) => {
  checkVariableForSource(row, ctx);
  checkPerformanceRateLevel(row, ctx);
  checkDemographicRules(row, ctx);
  checkRateValueRange(row, ctx);
});
